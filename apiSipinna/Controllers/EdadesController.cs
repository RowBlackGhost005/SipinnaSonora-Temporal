...
public class EdadesController:ControllerBase{
    ...
    public async Task<Edades> Get(int id){
        Edades? cat = null;
        try{
            cat = await _operations.ReadEdades(id);
        }catch(Exception e){
    ...
    [HttpPost]
    public async Task<Boolean> Post([FromBody]Edades cat){
        return await _operations.Create(cat);
    }

    [HttpPut]
    public async Task<Boolean> Put([FromBody]Edades cat){
        return await _operations.UpdateEdades(cat);
    }
